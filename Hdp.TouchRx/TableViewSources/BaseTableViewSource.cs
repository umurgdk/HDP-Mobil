...
{
    public abstract class BaseTableViewSource<TViewModel> : ReactiveUI.ReactiveTableViewSource<TViewModel>, IInformsEnd, IInformsEmpty
    {
        ...
        protected BaseTableViewSource(UITableView tableView, nfloat sizeHint)
            : base(tableView)
        {
            tableView.RowHeight = sizeHint;
            tableView.EstimatedRowHeight = sizeHint;
            this.WhenAnyValue(x => x.Data).Where(x => x != null).Select(x => x.Count == 0).Subscribe(_isEmptySubject.OnNext);
        }
